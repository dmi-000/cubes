...
def main():
                                    ...
                                    if sf <= 100:
                                        ...
                                        if rr is not None:
                                            for sgn in (1, -1):
                                                ...
                                                if max(abs(x) for pr in quad for x in pr) <= 512:
    ...
    print('countable classes (sf <= 100): %s'
          % {k: len(v) for k, v in sorted(bycls.items())}, flush=True)
            ...
            if t >= 723:
                ...
                if t > 727:
                    print('*** ABOVE 727 in Q(sqrt%d): %d %s' % (sf, t, q),
                          flush=True)
        print('  Q(sqrt%-3d): %5d configs, best %s, top %s'
              % (sf, len(cfgs), best[0],
                 {k: hist[k] for k in sorted(hist)[-6:]}), flush=True)
