...
// ---- INCREMENTAL BASE REUSE -------------------------------------------------
// Planes are laid out cube-major (pid = k*6 + j*2 + cs), so after clipping pids
// 0..6(n-1)-1 the cell list IS the arrangement of the first n-1 cubes. A search that
// holds a base fixed and varies only the last cube -- the arc sweeps, every menu
// search -- can therefore compute the base once and clip only six more planes per
// candidate. Measured base share: 57% at n=5, 63% at n=7, 71% at n=10, so the saving
// grows with n. `verts` is append-only, so restoring means truncating it.
static std::vector<Cell> g_baseCells;
// The VERTEX DATA must be saved, not just its length. exact_count_config() calls
// verts.clear() on entry, so resizing to the old length refilled it with
// default-constructed VData and every saved cell indexed into zeros -- which returned
// the BASE's count for every candidate after the first. The gate caught it; the
// timings taken before the fix were measuring skipped work.
static std::vector<VData> g_baseVertData;
static size_t g_baseVerts = 0;
static int    g_baseN = -1;
static bool   g_saveBase = false;
static bool   g_useBase  = false;
...
static Result exact_count_config(const std::vector<std::array<i64,4>>& quats) {
    ...
    int startPid = 0;
    if (g_useBase && g_baseN == n) {
        cells = g_baseCells;              // copy: the base is reused, never consumed
        verts = g_baseVertData;           // restore the DATA, not merely the length
        startPid = 6 * (n - 1);
    }
    for (int pid = startPid; pid < 6 * n; pid++) {
        if (g_saveBase && pid == 6 * (n - 1)) {
            g_baseCells = cells; g_baseVertData = verts;
            g_baseVerts = verts.size(); g_baseN = n;
        }
        g_signCache.bump();
        std::vector<Cell> nxt;
    ...
    {   // ---- ALWAYS report degenerate cells, not only when the guard fires.
        // A genuine 3-D cell has >= 4 faces; fewer means zero volume. If such
        // slivers are counted as regions in configurations that SUCCEED, counts
        // elsewhere are inflated and nothing would say so.
        int slivers = 0, sliver_pos_depth = 0;
        for (int i = 0; i < ncell; i++)
            if (cells[i].faces.size() < 4) {
                slivers++; if (labs[i] > 0) sliver_pos_depth++;
                if (slivers <= 40) {
                    fprintf(stderr, "[geom] cell %d depth %d verts", i, labs[i]);
                    for (auto& f : cells[i].faces)
                        for (int v : f.loop) {
                            const VData& V = verts[v];
                            fprintf(stderr, " %.6f,%.6f,%.6f",
                                    (double)V.X/(double)V.W, (double)V.Y/(double)V.W,
                                    (double)V.Z/(double)V.W);
                        }
                    fprintf(stderr, "\n");
                }
                if (slivers <= 8) {
                    fprintf(stderr, "[dbg] sliver cell %d: depth=%d faces=%zu pids=",
                            i, labs[i], cells[i].faces.size());
                    for (auto& f : cells[i].faces) {
                        int pid = f.pid;
                        if (pid >= 6 * g_n)
                            fprintf(stderr, " BOX(axis=%d,%s)", (pid - 6*g_n)/2,
                                    ((pid - 6*g_n) % 2 == 0) ? "+" : "-");
                        else
                            fprintf(stderr, " cube%d/axis%d/%s", pid/6, (pid%6)/2,
                                    (pid%2==0) ? "+" : "-");
                        fprintf(stderr, "[verts=%zu]", f.loop.size());
                    }
                    fprintf(stderr, "\n");
                }
            }
        fprintf(stderr, "[dbg] cells=%d  degenerate(<4 faces)=%d  of which depth>0=%d\n",
                ncell, slivers, sliver_pos_depth);
    }
    ...
    if (per_label.count(0) == 0 || per_label[0] != 1) {
        // ---- INSTRUMENTATION (cube_regions_dbg.cpp only; the validated source
        // is untouched). The guard says the engine's own bookkeeping left the
        // depth-0 cells in more than one union-find component, which geometry
        // forbids for concentric cubes. Print what the pieces are.
        std::map<int,std::vector<int>> byroot;
        for (int i = 0; i < ncell; i++) if (labs[i] == 0) byroot[find(i)].push_back(i);
        fprintf(stderr, "[dbg] ncell=%d  depth-0 cells=%zu  components=%zu\n",
                ncell, (size_t)std::count(labs.begin(), labs.end(), 0), byroot.size());
        for (auto& kv : byroot) {
            fprintf(stderr, "[dbg]   component root=%d  cells=%zu  sample cell %d:"
                    " faces=%zu  box-only-neighbours=", kv.first, kv.second.size(),
                    kv.second[0], cells[kv.second[0]].faces.size());
            int boxf = 0, intf = 0;
            for (auto& f : cells[kv.second[0]].faces)
                (f.pid >= 6 * g_n ? boxf : intf)++;
            fprintf(stderr, "%d internal=%d\n", boxf, intf);
        }
        // how many facet groups were skipped because they are box faces?
        long long skipped = 0;
        for (int ci = 0; ci < ncell; ci++)
            for (auto& f : cells[ci].faces) if (f.pid >= 6 * g_n) skipped++;
        fprintf(stderr, "[dbg] box-face facets seen (not merged): %lld\n", skipped);
...
int main(int argc, char** argv) {
    ...
    if (args[0] == "--base") {
        // --base "q;q;q" --candidates-stdin : count base+candidate for each line,
        // reusing the base arrangement. Equivalence with --quats is the gate.
        std::vector<std::array<i64,4>> base = parseQuats(args[1]);
        std::string line;
        bool first = true;
        while (std::getline(std::cin, line)) {
            if (line.empty()) continue;
            std::vector<std::array<i64,4>> add = parseQuats(line);
            if (add.size() != 1) { std::cout << "{\"error\":\"one quaternion per line\"}\n"; continue; }
            std::vector<std::array<i64,4>> cfg = base; cfg.push_back(add[0]);
            g_saveBase = first; g_useBase = !first;
            try {
                Result r = exact_count_config(cfg);
                std::cout << "{\"bounded\":" << r.total << ",\"by_depth\":{";
                bool f2 = true;
                for (auto& kv : r.by_depth) { if (!f2) std::cout << ","; f2 = false;
