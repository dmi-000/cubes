...
GEOMETRY.  A rank-13 flat is 2-dimensional (ambient 15, and the arrangement's rank
...
OUT = os.path.join(HERE, 'walk13_report.json')
...
def main():
    ...
    import epscount
    # WIDE eps engine (P167): the narrow one refused 20 of 20 rank-13 directions,
    # because a rank-13 flat is a 2-plane cut by 13 equations and contains no short
    ...
    m = len(W)
    masks = [int(x) for x in open(os.path.join(HERE, 'flats_727_rank13.txt')) if x.strip()]
    if limit:
        masks = masks[:limit]
    print('rank-13 flats: %d | ambient %d | walls %d | adapter gated at 727'
    ...
    for idx, mask in enumerate(masks):
        ...
        for r in rows2:
            nxt = []
            for ext in frontier:
                sub = [[s * rows2[i][k] for k in range(2)] for i, s in enumerate(ext)]
                for sgn in (1, -1):
                    if feasible_strict(sub + [[sgn * r[0], sgn * r[1]]], 2) is not None:
                        nxt.append(ext + [sgn])
            frontier = nxt
        for ext in frontier:
            sub = [[s * rows2[i][k] for k in range(2)] for i, s in enumerate(ext)]
            y2 = feasible_strict(sub, 2)
            if y2 is None:
                unev += 1
                continue
            y2 = simplify(sub, list(y2), 2)
