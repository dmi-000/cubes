...
int main(int argc, char** argv) {
    ...
    if (args.empty() || args[0] == "--help" || args[0] == "-h") {
        std::cerr <<
            "cube_regions_eps -- exact bounded-region counter for N congruent concentric\n"
            "cubes, rotations optionally in Z[sqrt d] (d given at runtime).\n"
            "usage:\n"
            "  cube_regions_eps --selftest\n"
            "  cube_regions_q2 [--n K] --seed S                 (d=0 only)\n"
            "  cube_regions_q2 [--n K] --seeds A B               (d=0 only)\n"
            "  cube_regions_eps [--d D] --quats '...'   (K ';'-separated groups)\n"
            "  cube_regions_eps [--d D] --quats-stdin   (one config per line)\n"
