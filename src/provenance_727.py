...
def main():
    ...
    WANT = _c.defaultdict(set)
    for _l in open('mixed_q2_hits.jsonl'):
        _r = _j.loads(_l)
        if _r['total'] == 727:
            WANT[_r['d']].add(tuple(tuple(t) for t in _r['quat']))
    print('tracking %d known 727 configs in %d fields'
          % (sum(len(v) for v in WANT.values()), len(WANT)), flush=True)
    prov = []
                                    ...
                                    if sf <= 30000000:
                                        ...
                                        if rr is not None:
                                            for sgn in (1, -1):
                                                ...
                                                if quad in WANT.get(sf, ()):
                                                    prov.append({
                                                        'd': sf,
                                                        'quat': [list(t) for t in quad],
                                                        'cubes_planes': [pi, pj],
                                                        'cube_quadric': qi,
                                                        'plane_idx': [ip1, ip2],
                                                        'quadric_idx': iq})
    ...
    import json as _j2
    _j2.dump(prov, open('provenance_727.json', 'w'))
    print('provenance records: %d' % len(prov), flush=True)
