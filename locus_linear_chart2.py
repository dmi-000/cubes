...
def extract_planes():
    ...
    per = pickle.load(open('locus_polys_chart2.pkl', 'rb'))
    out = {}
    for j in range(5):
        S = set()
        for P, _ in per[j]:
...
def to_quat(pt):
    ...
    q = (int(pt[0] * den), den, int(pt[1] * den), int(pt[2] * den))
    g = 0
    for x in q:
        g = math.gcd(g, abs(x))
...
def main():
    ...
    out = open('locus_linear_chart2.jsonl', 'a')
    best, hist = (0, None), {}
    B = 500
    ...
    json.dump({'hist': {str(k): v for k, v in hist.items()},
              ...
              open('locus_linear_chart2_summary.json', 'w'), indent=1)
