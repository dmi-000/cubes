...
static Result exact_count_config(const std::vector<std::array<i64,4>>& quats) {
    ...
    // ---- DISCARD ZERO-VOLUME CELLS. A 3-D cell has >= 4 faces; the clipper can
    // emit 1- and 2-face cells, which enclose no volume and are not regions. They
    // are what trips "outside must be a single region" (P180 Addendum 1). Dropping
    // them is a symptom fix, used here only to obtain the count this configuration
    // would have had; the clipper emitting them at all remains undiagnosed.
    std::vector<char> keep(ncell, 1);
    int dropped = 0;
    for (int i = 0; i < ncell; i++)
        if (cells[i].faces.size() < 4) { keep[i] = 0; dropped++; }
    fprintf(stderr, "[fix] dropped %d degenerate cells of %d\n", dropped, ncell);
    ...
    for (auto& kv : groups) {
        ...
        if (cs.size() == 1) continue;      // partner was a discarded sliver
    ...
    for (int i = 0; i < ncell; i++) if (keep[i]) comps.insert({labs[i], find(i)});
    std::map<int,int> per_label;
