...
```text
Empirical search: among compounds of 6 congruent, concentric cubes (each cube an arbitrary rotation of [-1,1]^3 about the shared center), which configurations maximize the number of BOUNDED regions that the 6 cube surfaces cut space into? Work in ~/carroll.

EXISTING TOOL (do not modify its validated logic; extend additively):
~/carroll/cube_compound_regions.py — a voxel region counter. Read its docstring first. Key API:
...
STEP 2 — write a NEW file ~/carroll/six_cube_search.py that imports from cube_compound_regions and adds a way to count arbitrary configurations (list of 6 rotation matrices, e.g. via scipy Rotation.random with fixed seeds or your own Rodrigues code — there is a rodrigues() in the module). Simplest approach: monkeypatch/extend compound() lookup or write a labels/count variant taking matrices directly (copy the ~20-line counting wrapper if easier; keep the small->big merge policy and tau=3R exactly as in count()).
