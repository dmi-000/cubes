...
BASES = {
    4: [(1, 0, 0, 0), (0, 5, 3, 2), (1, -4, -1, 1)],              # 183 minus one
...
def main():
    ...
    for tri in itertools.combinations(idx, min(3, len(base))):
        for qi in tri:
            ...
            if len(others) < 2:
                continue
            pi, pj = others
            for ip1, P1 in enumerate(planes[pi]):
                for ip2, P2 in enumerate(planes[pj]):
                    L = line_of(P1, P2)
                    if L is None:
                        continue
                    p0, dd = L
                    ...
                    for iq, co in enumerate(quads[qi]):
                        nsys += 1
                        ...
                        for sgn in (1, -1):
                            ...
                            if max(abs(x) for pr in quad for x in pr) <= MAXCOMP:
                                ...
                                PROV.setdefault((sf, quad), (pi, pj, qi, ip1, ip2, iq))
    ...
    out = open('mixed_base_n%d_hits.jsonl' % n, 'a')
    ...
    hits = [(d, q) for d, q in PROV if any(
        r['d'] == d and tuple(tuple(t) for t in r['quat']) == q
        for r in [json.loads(l) for l in open('mixed_base_n%d_hits.jsonl' % n)]
        if r['total'] >= rec)]
    who = _c.Counter()
    for k in hits:
        pi, pj, qi, _, _, _ = PROV[k]
        who[('planes', tuple(sorted((pi, pj))))] += 1
        who[('quadric', qi)] += 1
    print('\nprovenance of the record-reaching configs:', dict(who), flush=True)
