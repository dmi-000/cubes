...
| `ALGEBRAIC_SEARCH.md` | spec+report | The algebraic (not numeric) search: solves incidence equations symbolically (Wolfram) — 1-parameter exact wall-mapping and multi-constraint GroebnerBasis solving — then counts each config exactly. Records live at high-multiplicity *point* concurrences (coincident corners = shared axes); the "9-fold sweet spot" it reports is a property of 723 and not of records generally — 727 beats it with max concurrence 6 ([P227](LEDGER.md#p227)). Working-principles doc for `algebraic_search.wl`, `algebraic_demo.wl`, `algebraic_groebner.wl`, `algebraic_groebner2.wl`, `algebraic_bridge.py`, `cad_probe_n2.wl` (CAD feasibility probe; verdict in C45_notes §11). |
